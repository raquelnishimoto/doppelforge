...
const Errors = {
  ghostAlias: (typeName: string, aliasTarget: string) =>
    `Alias "${typeName}" points to a non-existent type: "${aliasTarget}". ` +
    `This usually means "${aliasTarget.split('#')[0]}" is outside your configured include globs.`,
  ambiguous: (typeName: string, qualifiedIds: string[]) =>
    `Type "${typeName}" is ambiguous. Use a qualified id instead:\n` +
